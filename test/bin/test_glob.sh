...
(
	...
	cmp -s "$globDst" "$cmdBase/../data/glob/glob.txt" \
	...
	cmp -s "$globDst" "$cmdBase/../data/glob/glob_txt.txt" \
		|| test_fail_psst $LINENO
	...
	cmp -s "$globDst" "$cmdBase/../data/glob/glob_space.txt" \
		|| test_fail_psst $LINENO
	...
	cmp -s "$globDst" "$cmdBase/../data/glob/glob_txt_space.txt" \
		|| test_fail_psst $LINENO
)
...
(
	...
	cmp -s "$globDst" "$cmdBase/../data/glob/glob_3.txt" \
		|| test_fail_psst $LINENO
	...
	cmp -s "$globDst" "$cmdBase/../data/glob/glob_space_3.txt" \
		|| test_fail_psst $LINENO
